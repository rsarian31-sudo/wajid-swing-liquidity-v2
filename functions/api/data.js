...
export async function onRequest({request,env,waitUntil}){
  ...
  try{
    ...
    const summary={totalTrades:trades.length,wins:completed.filter(t=>t.result==='WIN'||t.result==='FULL TP HIT').length,losses:completed.filter(t=>t.result==='LOSS').length,open:activeTrades.length,totalR:trades.reduce((s,t)=>s+Number(t.realizedR||0),0)};
