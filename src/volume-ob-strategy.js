...
const CONFIG = { stLen: 50, stMult: 3.5, pivotLen: 7, bullVolPct: 50, bearVolPct: 50, showBullRetest: true, showBearRetest: true, deleteOnBreak: true };
...
function pivotLow(c, i, len) {
  ...
  for (let j = p - len; j <= p + len; j++) if (j !== p && c[j].low < c[p].low) return null;
  return c[p].low;
}
...
function pivotHigh(c, i, len) {
  const p = i - len;
  if (p < len || p + len >= c.length) return null;
  for (let j = p - len; j <= p + len; j++) if (j !== p && c[j].high > c[p].high) return null;
  return c[p].high;
}
...
function windowBuyRatio(c, i, lookbackLen) {
  ...
  for (let j = 0; j <= lookbackLen; j++) {
    ...
    if (c[k].close >= c[k].open) buy += v; else sell += v;
  }
...
function buildTrend(c) {
  ...
  for (let i = 0; i < c.length; i++) {
    if (atr[i] == null) { trend[i] = 1; continue; }
...
export function analyzeVolumeOB(c) {
  ...
  const createZone = (trend, pivotIndex, i) => {
    ...
    if (!(atr > 0)) return;
    const body = trend === 1 ? Math.min(c[pivotIndex].open, c[pivotIndex].close) : Math.max(c[pivotIndex].open, c[pivotIndex].close);
    const top = trend === 1 ? body : body + atr;
    const bottom = trend === 1 ? body - atr : body;
  ...
  for (let i = 0; i < c.length; i++) {
    ...
    if (active && CONFIG.deleteOnBreak) {
      const isBroken = active.trend === 1 ? c[i].high < active.bottom : c[i].low > active.top;
      if (isBroken) closeActive(i, true);
    }

    const marketChange = i > 0 && t.trend[i] !== t.trend[i - 1];
    const prev = i > 0 ? c[i - 1] : null;
    const buyRetest = !!active && active.trend === 1 && CONFIG.showBullRetest && pl == null && active.buyRatio >= CONFIG.bullVolPct / 100 && !marketChange && !!prev && prev.low <= active.top && c[i].low > active.top;
    const sellRetest = !!active && active.trend === -1 && CONFIG.showBearRetest && ph == null && active.sellRatio >= CONFIG.bearVolPct / 100 && !marketChange && !!prev && prev.high >= active.bottom && c[i].high < active.bottom;

    if (buyRetest || sellRetest) {
      const direction = buyRetest ? 'BUY' : 'SELL';
      signals.push({ direction, time: c[i].time, index: i, price: c[i].close, entry: c[i].close, confirmed: true, source: 'volume-ob', ob: { ...active }, buyPercent: Math.round(active.buyRatio * 100), sellPercent: Math.round(active.sellRatio * 100) });
    }
  }
